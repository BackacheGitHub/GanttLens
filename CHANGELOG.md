...
### Fixed
- GUI: 修复甘特图分组头上下出现黑色填充的问题（时间轴绘制范围限制在 header 区域）
- GUI: 修复分组头参与隔行换色，文字使用深色粗体（#424242）确保可读性
- GUI: 修复左侧标签列分组头文字颜色与背景冲突导致不可见的问题
- Test: 扩展 MainFxmlTest 验证默认示例（含分组头）能正常加载

### Changed
- Docs: 更新 architecture.md 说明 GanttCanvasView 当前测试策略（集成测试 + 人工验证）
